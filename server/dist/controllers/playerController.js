...
function generatePlayerData(nickname, id, room = 'PokerStars') {
...
function getPlayerResults(req, res) {
    const { id } = req.params;
    ...
    const results = Array.from({ length: limit }).map((_, i) => {
        ...
        if (filterType && item.type !== filterType)
            return null;
        if (filterRoom && item.room !== filterRoom)
            return null;
        return item;
    });
...
function getRankings(req, res) {
    ...
    let filtered = scored;
    if (room)
        filtered = filtered.filter((p) => p.room === room);
    if (country)
...
exports.subscribers = new Set();
const trainingsMap = new Map();
function todayString() {
    return new Date().toISOString().slice(0, 10);
}
function ensureUsageFor(userId) {
...
async function generateScenario(req, res) {
    ...
    const freeLimit = 5;
    if (!exports.subscribers.has(userId) && usage.count >= freeLimit) {
        return res.status(403).json({ ok: false, error: 'limit', message: 'Limite diário atingido' });
    }
    // consume one training
    ...
    const board = street !== 'Pré-flop' ? [randCard(), randCard(), randCard()] : [];
    ...
    if (OPENAI) {
        try {
                ...
                try {
                    ...
                    try {
                        ...
                        if (parsed && (parsed.villainRange || parsed.correctAction || typeof parsed.ev === 'number')) {
                            if (parsed.villainRange)
                                scenario.villainRange = parsed.villainRange;
                            if (parsed.correctAction)
                                scenario.correctAction = parsed.correctAction;
                            if (typeof parsed.ev === 'number')
                                scenario.ev = parsed.ev;
                            if (parsed.explanation)
                                scenario.aiExplanation = parsed.explanation;
                            usedModel = model;
                            break;
                        }
                    }
            ...
            if (usedModel) {
                scenario.aiModelUsed = usedModel;
    ...
    const aiModelUsed = scenario.aiModelUsed || null;
    const aiExplanation = scenario.aiExplanation || null;
    return res.json({ ok: true, scenario, aiModelUsed, aiExplanation, remaining: exports.subscribers.has(userId) ? -1 : Math.max(0, freeLimit - usage.count) });
}
function recordResult(req, res) {
    ...
    if (!scenario)
        return res.status(404).json({ ok: false, error: 'scenario_not_found' });
    ...
    const existing = trainingsMap.get(statKey) || [];
    existing.push({ ...result, scenario });
    trainingsMap.set(statKey, existing);
...
function getUsage(_req, res) {
    const userId = (_req.query && _req.query.user) || 'anon';
    const usage = ensureUsageFor(userId);
    const freeLimit = 5;
    const remaining = exports.subscribers.has(userId) ? -1 : Math.max(0, freeLimit - usage.count);
    res.json({ ok: true, remaining, used: usage.count });
}
function getStats(_req, res) {
    const userId = (_req.query && _req.query.user) || 'anon';
    const statKey = `${userId}::results`;
    const results = trainingsMap.get(statKey) || [];
...
function subscribe(req, res) {
    const userId = (req.body && req.body.user) || (req.query && req.query.user) || 'anon';
    exports.subscribers.add(userId);
