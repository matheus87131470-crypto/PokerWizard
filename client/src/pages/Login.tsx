...
export default function Login() {
  ...
  const [isWakingUp, setIsWakingUp] = useState(false); // Estado para wake-up do servidor
  ...
  // GOOGLE AUTH COM WAKE-UP INTELIGENTE
  const handleGoogleAuth = async () => {
    try {
      setIsWakingUp(true);
      setError('');

      const API_BASE = (import.meta as any).env.VITE_API_BASE || 'https://pokerwizard-api.onrender.com';
      
      // 1. Ping no health check para acordar o servidor
      const healthResponse = await fetch(`${API_BASE}/api/auth/health`, {
        method: 'GET',
      ...
      // 2. Aguardar 1s para garantir que servidor está pronto
      ...
      if (err.name === 'TimeoutError') {
        setError('Servidor está demorando muito. Tente novamente em 1 minuto.');
      } else {
  ...
  return (
    ...
    >
      <div className="card" style={{ maxWidth: 420, width: '100%', maxHeight: '90vh', overflowY: 'auto' }}>
        ...
        <button
          ...
          style={{
            ...
            background: isWakingUp 
              ? 'linear-gradient(145deg, rgba(168, 85, 247, 0.2), rgba(107, 33, 168, 0.2))'
              : 'linear-gradient(145deg, rgba(255, 255, 255, 0.95), rgba(240, 240, 240, 0.95))',
            border: '1px solid rgba(168, 85, 247, 0.3)',
            borderRadius: 12,
            color: isWakingUp ? '#94a3b8' : '#333',
        ...
        >
          {/* Google Logo SVG ou Loading Spinner */}
          {isWakingUp ? (
            <div
              style={{
                width: 20,
                ...
                borderTop: '3px solid rgb(168, 85, 247)',
                borderRadius: '50%',
          ...
          {isWakingUp ? 'Conectando ao servidor...' : 'Continuar com Google'}
        </button>
