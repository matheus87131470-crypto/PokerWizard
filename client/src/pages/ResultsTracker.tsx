...
export default function ResultsTracker() {
  ...
  return (
    <div style={{ padding: '24px 20px', maxWidth: 1200, margin: '0 auto' }}>
      ...
      {/* 3️⃣ TABELA PREMIUM DE SESSÕES - FLUTUANTE FULL WIDTH */}
      {sessions.length > 0 && (
        <div className="chart-floating" style={{ maxWidth: 1400, margin: '0 auto 32px', width: '100%', padding: '0 5%', position: 'relative' }}>
          <SessionsTable sessions={sessions} isBlurred={shouldBlurChart} onUpgrade={() => navigate('/premium')} />
        </div>
      )}
...
const SessionsTable = ({ sessions, isBlurred, onUpgrade }: SessionsTableProps) => {
  ...
  if (period === '7d') {
    filteredSessions = sessions.filter(s => s.timestamp >= now - 7 * dayMs);
  } else if (period === '30d') {
    filteredSessions = sessions.filter(s => s.timestamp >= now - 30 * dayMs);
  }
  ...
  const sortedSessions = [...filteredSessions].sort((a, b) => {
    if (sortBy === 'date') {
      ...
      return sortOrder === 'desc' ? b.net - a.net : a.net - b.net;
    }
  });

  const handleSort = (field: 'date' | 'result') => {
    if (sortBy === field) {
  ...
  // Calcular totais
  const totalGains = sortedSessions.reduce((acc, s) => acc + s.gains, 0);
  const totalLosses = sortedSessions.reduce((acc, s) => acc + s.losses, 0);
  const totalNet = sortedSessions.reduce((acc, s) => acc + s.net, 0);
  ...
  return (
    <div style={{ position: 'relative' }}>
      ...
      <div style={{ 
        ...
        <div>
          <h3 style={{ fontSize: 24, fontWeight: 700, color: '#f8fafc', marginBottom: 6 }}>
            📊 Histórico de Sessões
          </h3>
          <p style={{ fontSize: 13, color: '#94a3b8', margin: 0 }}>
            {sortedSessions.length} {sortedSessions.length === 1 ? 'sessão registrada' : 'sessões registradas'}
          </p>
        </div>
          ...
          {(['7d', '30d', 'all'] as const).map(p => (
            <button
              ...
              style={{
                ...
                fontWeight: 600,
                color: period === p ? '#fff' : '#94a3b8',
                background: period === p ? 'rgba(59, 130, 246, 0.2)' : 'rgba(15, 23, 42, 0.4)',
                border: period === p ? '1px solid rgba(59, 130, 246, 0.4)' : '1px solid rgba(71, 85, 105, 0.2)',
                borderRadius: 8,
                cursor: 'pointer',
                transition: 'all 0.2s'
              }}
      ...
      <div style={{ 
        ...
        <div style={{
          ...
          <div style={{ fontSize: 11, color: '#94a3b8', marginBottom: 4, textTransform: 'uppercase', letterSpacing: '0.5px', fontWeight: 600 }}>
            Total Ganhos
          </div>
        ...
        <div style={{
          ...
          border: totalNet >= 0 ? '1px solid rgba(59, 130, 246, 0.2)' : '1px solid rgba(239, 68, 68, 0.2)',
          borderRadius: 10,
          padding: '12px 16px'
        }}>
          <div style={{ fontSize: 11, color: '#94a3b8', marginBottom: 4, textTransform: 'uppercase', letterSpacing: '0.5px', fontWeight: 600 }}>
          ...
          <div style={{ fontSize: 18, fontWeight: 700, color: totalNet >= 0 ? '#3b82f6' : '#ef4444' }}>
      ...
      <div style={{ 
        ...
        <div style={{ 
          display: 'grid',
          gridTemplateColumns: '140px 1fr 1fr 140px 100px',
          gap: 16,
          padding: '14px 20px',
          ...
          >
            Resultado {sortBy === 'result' && (sortOrder === 'desc' ? '↓' : '↑')}
          </button>
          <div style={{ fontSize: 11, fontWeight: 700, color: '#94a3b8', textTransform: 'uppercase', letterSpacing: '0.5px', textAlign: 'center' }}>
            Status
          </div>
        </div>
          ...
          {sortedSessions.map((session, index) => (
            <div
              ...
              style={{
                ...
                gap: 16,
                padding: '16px 20px',
                borderBottom: index < sortedSessions.length - 1 ? '1px solid rgba(71, 85, 105, 0.1)' : 'none',
                transition: 'all 0.2s',
                cursor: 'default'
              }}
              onMouseEnter={(e) => {
            ...
            >
              ...
              <div style={{ fontSize: 14, fontWeight: 600, color: '#10b981' }}>
                +{session.gains.toLocaleString('pt-BR', { style: 'currency', currency: 'BRL' })}
              </div>

              {/* Perdas */}
              <div style={{ fontSize: 14, fontWeight: 600, color: '#ef4444' }}>
                -{session.losses.toLocaleString('pt-BR', { style: 'currency', currency: 'BRL' })}
              </div>
              ...
              <div style={{ 
                fontSize: 15, 
                fontWeight: 700, 
                color: session.net >= 0 ? '#3b82f6' : '#ef4444'
              }}>
              ...
              <div style={{ textAlign: 'center' }}>
                <span style={{
                  ...
                  fontSize: 11,
                  fontWeight: 700,
                  background: session.net >= 0 ? 'rgba(16, 185, 129, 0.15)' : 'rgba(239, 68, 68, 0.15)',
                  color: session.net >= 0 ? '#10b981' : '#ef4444',
                  border: session.net >= 0 ? '1px solid rgba(16, 185, 129, 0.3)' : '1px solid rgba(239, 68, 68, 0.3)'
                }}>
                  {session.net >= 0 ? '✓ Lucro' : '✗ Prejuízo'}
                </span>
              </div>
            </div>
          ))}
